    ...
    if(btnval.equalsIgnoreCase("create"))
     ...
     if(btnval.equalsIgnoreCase("withdraw"))
     {
    	...
    	try
    	{
    		...
    		Class.forName("oracle.jdbc.driver.OracleDriver");
    		Connection conn=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "bank","bank");
    	    PreparedStatement psmt=conn.prepareStatement("update information set balance = balance - ? where account_no=?");
    	    psmt.setString(1,t2);
    	    psmt.setString(2,t1);
    	   	psmt.executeQuery();
    	   	PreparedStatement psm=conn.prepareStatement("insert into tranhistory(account_no,amount,payment_method,payment_type) values (?,?,?,'Withdraw')");
    	    psm.setString(1,t1);
    	    psm.setString(2,t2);
    	    psm.setString(3,t3);
    	   	psm.executeQuery();
    	    out.println("<script>alert('Withdraw success....')</script>");
