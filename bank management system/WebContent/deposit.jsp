    ...
    if(btnval.equalsIgnoreCase("create"))
     ...
     if(btnval.equalsIgnoreCase("deposit"))
     {
		...
		try
    	{
    	    ...
    	    psmt.setString(2,t1);
    	   	psmt.executeQuery();
    	   	PreparedStatement psm=conn.prepareStatement("insert into tranhistory(account_no,amount,payment_method,payment_type) values (?,?,?,'Deposit')");
    	    psm.setString(1,t1);
    	    psm.setString(2,t2);
    	    psm.setString(3,t3);
    	   	psm.executeQuery();
    	    out.println("<script>alert('Deposit success....')</script>");
